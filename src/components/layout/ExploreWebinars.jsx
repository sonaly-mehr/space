...
const ExploreWebinars = () => {
  ...
  return (
    <div className="container py-10 lg:py-20">
      <p className="tracking-[-7px] xl:tracking-[-14px] text-[5rem] lg:text-[9.375rem] leading-[100%] lg:leading-[10rem] text-primary font-light">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-x-8 gap-y-8 lg:gap-y-12 responsive-grid-cols">
        {articles?.map((article) => (
          <div className="relative article-content w-full overflow-hidden rounded-[1.25rem] cursor-pointer">
            ...
            />
            <div className="absolute left-4 md:left-8 bottom-4 pr-4 md:pr-8 md:bottom-8 lg:left-8 lg:bottom-8 lg:pr-8 z-10">
              <h5 className="text-xl lg:text-2xl lg:leading-[36px] text-white mb-5 font-light">
                {article?.heading}
              </h5>
              <Link
                href={article?.url}
                className="text-primary text-sm md:text-base bg-white rounded-lg font-medium normal-case px-4 py-2 flex gap-1.5 items-center w-fit"
              >
